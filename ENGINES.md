...
| **Aktuální** | v3.4 — má: depth 2+ID + α-β + quiescence + endgame king-tropism + TT + PV + killer/history + PSQT tapered eval (PeSTO) + **mobility eval (pseudo-legal)**. **Sparring +191 Elo nad v3.3** (decisive, 100 partií). Kumulativně +604 Elo nad v3.2 (PSQT + mobility eval). |
| **Snapshoty** | v2.7 (textbook depth-2 + MVV-LVA), v3.1 (decisive TT/PV milestone), v3.2 (pre-PSQT eval baseline), v3.3 (pre-mobility, PSQT only). |

...
- **Import partií z Lichess** na `/import` — stáhne public partie uživatele přes oficiální export API (bez OAuth, 1-500 partií/request). **Default inkrementální** (Lichess `since=<ms>` filter — stahuje jen partie novější než poslední v DB, žádný zbytečný transfer); checkbox „Force full re-sync" pro repair. Idempotentní jako safety net (INSERT OR IGNORE). Uložení do SQLite `data/chesslab.db`.

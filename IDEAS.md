...
- ~~**v3.4: Mobility eval (pseudo-legal)**~~ — **HOTOVO 2026-05-19**, viz DONE. `board.attacks_mask(sq) & ~own_occupied`, popcount, weights per piece (N=4, B=3, R=2, Q=1, P/K=0). **Decisive +191 Elo nad v3.3** (100 partií, CI [+112, +269], score 75 %, 16 remíz). V rámci očekávání +100-200, diminishing returns kvůli overlap s PSQT — oba sdílí signál „centrum = dobrá pozice".
- **v3.5: King safety eval** *(Minimax-scoped)* — pawn shield + attackers count kolem krále. Orthogonal k PSQT/mobility (řeší jiný aspekt — bezpečnost krále vs centralization). Očekávaný gain +100-200 Elo. Komplexnější impl (víc edge cases v MG vs EG, castling state).
- **v3.5 alt: Pawn structure eval** *(Minimax-scoped)* — isolated, doubled, passed pawns. Jednodušší impl než king safety. Orthogonal k existujícím evalům.

...
- **v3.2: Killer moves / history heuristic** — non-capture ordering nad MVV-LVA. PV pokrývá jen prev best per pozice; killer table (2 killer moves per ply, refresh při β-cutoff) a history heuristic (`history[from][to] += depth²` při cutoff, sort by descending history) pomohou ordering ostatních quiet moves. Standard chess engine pattern. Odhad +30-80 Elo, ~60 řádků.
- **v3.2 alt: Aspiration windows** — úzké α-β okno kolem prev iter score, re-search při miss. Zrychlí deep iterace o ~20 %. ~30 řádků. Kombinovatelné s killer/history.
- **v3.2 alt: Mate-distance scoring** — `_MATE_SCORE - ply` místo flat. Zatím engine vidí jen mate-in-1 (všechny mate scores jsou stejné). Distance scoring → preferuje rychlejší mat. Vyžaduje TT mate-distance adjustment na store/probe (komplikovanější než zní).
